...
local function addBlur(parent)
	local blur = Instance.new('UIShadow')
	blur.Name = 'Blur'
	blur.BlurRadius = UDim.new(0, 12)
...
function mainapi:Load(skipgui, profile)
	...
	local savedata = profilePath and loadJson(profilePath) or nil
	if type(savedata) == 'table' then
		...
		for name, saved in (savedata.Modules or {}) do
			...
			if object then
				if saved.Options then self:LoadOptions(object, saved.Options) end
				if object.SetBind then object:SetBind(saved.Bind or {}) end
	...
	self.Loading = nil
	if not isfile(guiPath) or not profilePath then self:Save() end
...
function mainapi:LoadOptions(object, savedoptions)
	for name, saved in (type(savedoptions) == 'table' and savedoptions or {}) do
...
function guibind:SetBind(keys, mouse)
	keys = bindKeys(keys, {'RightShift'})
	self.Bind = table.clone(keys)
