...
targetbutton.BackgroundColor3 = uipallet.PanelLight
...
bkg.BackgroundColor3 = uipallet.Panel
...
icon.ImageColor3 = uipallet.Muted
icon.Parent = bkg
...
function optionapi:Toggle()
	...
	tween:Tween(bkg, uipallet.Tween, {
		BackgroundColor3 = self.Enabled and Color3.fromHSV(mainapi.GUIColor.Hue, mainapi.GUIColor.Sat, mainapi.GUIColor.Value) or uipallet.Panel
	})
	tween:Tween(icon, uipallet.Tween, {
		ImageColor3 = self.Enabled and Color3.new(1, 1, 1) or uipallet.Muted
	})
	if tooltipicon then
		tooltipicon:Destroy()
	end
...
targetbutton.MouseLeave:Connect(function()
	if not optionapi.Enabled then
		tween:Tween(bkg, uipallet.Tween, {
			BackgroundColor3 = uipallet.Panel
		})
		tween:Tween(icon, uipallet.Tween, {
			ImageColor3 = uipallet.Muted
		})
	end
end)
